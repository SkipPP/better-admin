...
export function MonthView({
  ...
  const handleEventClick = useCallback(
    (event: Meeting, e: React.MouseEvent) => {
      e.stopPropagation();
      onEventSelect(event);
    },
    [onEventSelect],
  ...
  // Pre-calculate all event data for each day
  ...
  const renderEventList = useCallback(
    (day: Date, allDayEvents: Meeting[], visibleCount: number | undefined) => {
      if (!visibleCount) return null;

      return sortEvents(allDayEvents).map((event, index) => {
  ...
  return (
    <>
      ...
      <div className="grid flex-1 auto-rows-fr" role="grid">
        {weeks.map((week, weekIndex) => (
          ...
          >
            {week.map((day, dayIndex) => {
              const dayData = dayEventData[weekIndex * 7 + dayIndex];
              const isReferenceCell = weekIndex === 0 && dayIndex === 0;
              const visibleCount = isMounted
                ? getVisibleEventCount(dayData.allDayEvents.length)
                : undefined;
              const hasMore =
                visibleCount !== undefined && dayData.allDayEvents.length > visibleCount;
              const remainingCount = hasMore
                ? dayData.allDayEvents.length - visibleCount
                : 0;
              ...
              return (
                <div
                  ...
                  data-outside-cell={!dayData.isCurrentMonth || undefined}
                  role="gridcell"
                ...
                >
                  ...
                  >
                    <div className="group-data-today:bg-primary group-data-today:text-primary-foreground mt-1 inline-flex size-6 items-center justify-center rounded-sm text-sm">
                      {format(day, "d")}
                    </div>
                    ...
                    >
                      {renderEventList(day, dayData.allDayEvents, visibleCount)}
                        ...
                        <Popover modal>
                          <PopoverTrigger asChild>
                            <button
                              ...
                              aria-label={`Show ${remainingCount} more events for ${format(day, "MMMM d, yyyy")}`}
                            >
                              <span>
                                + {remainingCount}{" "}
                          ...
                          >
                            <div className="space-y-2">
                              ...
                              <div className="space-y-1" role="list">
                                {sortEvents(dayData.allEvents).map((event) => {
                                  const eventStart = new Date(event.start);
                                  const eventEnd = new Date(event.end);
                                  const isFirstDay = isSameDay(day, eventStart);
                                  const isLastDay = isSameDay(day, eventEnd);
