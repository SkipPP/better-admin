...
export const FilterIcon = ({
  ...
  type: FilterType | Status | Task["status"] | Assignee | Labels | Priority;
}) => {
  switch (type) {
    ...
    case Status.BACKLOG:
    case "BACKLOG":
      return <CircleDashed className="text-muted-foreground size-3.5" />;
    case Status.TODO:
    case "TODO":
      return <Circle className="text-primary size-3.5" />;
    ...
    case "CANCELLED":
      return <CircleX className="text-muted-foreground size-3.5" />;
    ...
    case Labels.BUG:
      return <div className="size-2.5 rounded-full bg-red-400" />;
    case Labels.FEATURE:
      return <div className="size-2.5 rounded-full bg-blue-400" />;
    case Labels.HOTFIX:
      return <div className="size-2.5 rounded-full bg-amber-400" />;
    case Labels.RELEASE:
      return <div className="size-2.5 rounded-full bg-green-400" />;
  }
};
...
export const assigneeFilterOptions: FilterOption[] = Object.values(Assignee).map(
  (assignee) => ({
    name: assignee,
...
export const labelFilterOptions: FilterOption[] = Object.values(Labels).map((label) => ({
  name: label,
  icon: <FilterIcon type={label} />,
}));

export const priorityFilterOptions: FilterOption[] = Object.values(Priority).map(
  (priority) => ({
    name: priority,
...
export const dateFilterOptions: FilterOption[] = Object.values(DueDate).map((date) => ({
  name: date,
  icon: undefined,
}));
...
const FilterOperatorDropdown = ({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="bg-muted hover:bg-muted/50 text-muted-foreground hover:text-primary shrink-0 p-2 transition">
        {operator}
      </DropdownMenuTrigger>
...
const FilterValueCombobox = ({
  ...
  return (
    ...
    >
      <PopoverTrigger className="bg-muted hover:bg-muted/50 text-muted-foreground hover:text-primary shrink-0 rounded-none p-2 transition">
      ...
      {filters
        ...
        .map((filter) => (
          <div key={filter.id} className="flex items-center gap-[1px] text-xs">
            <div className="bg-muted flex shrink-0 items-center gap-1.5 rounded-l p-2">
            ...
            <Button
              ...
              className="bg-muted text-muted-foreground hover:text-primary hover:bg-muted/50 h-8 w-8 shrink-0 rounded-l-none rounded-r-sm transition"
            >
              <X className="size-3" />
            </Button>
          </div>
